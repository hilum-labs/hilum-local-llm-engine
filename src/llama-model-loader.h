...
struct llama_model_loader {
    ...
    struct llama_tensor_weight {
        ...
        // Buffer-backed variant: accepts total buffer size instead of file pointer
        llama_tensor_weight(size_t buf_size, uint16_t idx, const struct gguf_context * gguf_ctx, ggml_tensor * tensor) : idx(idx), tensor(tensor) {
            const int tensor_idx = gguf_find_tensor(gguf_ctx,  ggml_get_name(tensor));
            if (tensor_idx < 0) {
                throw std::runtime_error(format("tensor '%s' not found in the model", ggml_get_name(tensor)));
            }
            ...
            offs = gguf_get_data_offset(gguf_ctx) + gguf_get_tensor_offset(gguf_ctx, tensor_idx);
            if (offs + ggml_nbytes(tensor) < offs || offs + ggml_nbytes(tensor) > buf_size) {
    ...
    // Buffer-backed loading (when data != nullptr, reads from buffer instead of files)
    ...
    // Buffer-backed constructor for loading models from memory.
    // The buffer must outlive the model loader and any loaded model.
    llama_model_loader(
        const void * data,
        size_t size,
        bool check_tensors,
