...
llama_model_loader::llama_model_loader(
    ...
    meta.reset(gguf_init_from_buffer(data, size, params));
    if (!meta) {
        throw std::runtime_error(format("%s: failed to load model from buffer (%zu bytes)", __func__, size));
    }
    ...
    // Store buffer pointer for tensor data reads
    this->buf_data = static_cast<const uint8_t *>(data);
    this->buf_size = size;

    // Build weights map from tensors in the GGUF context
    for (ggml_tensor * cur = ggml_get_first_tensor(ctx); cur; cur = ggml_get_next_tensor(ctx, cur)) {
        std::string tensor_name = std::string(cur->name);
        if (weights_map.find(tensor_name) != weights_map.end()) {
            throw std::runtime_error(format("invalid model: tensor '%s' is duplicated", ggml_get_name(cur)));
        ...
        n_elements += ggml_nelements(cur);
        n_bytes    += ggml_nbytes(cur);
        weights_map.emplace(tensor_name, llama_tensor_weight(size, 0, meta.get(), cur));
    }

    // Split files not supported for buffer loading
    uint16_t n_split = 0;
    get_key(llm_kv(LLM_KV_SPLIT_COUNT), n_split, false);
    if (n_split > 1) {
        throw std::runtime_error("buffer loading does not support split models");
    }
    ...
    LLAMA_LOG_INFO("%s: loaded meta data with %d key-value pairs and %d tensors from buffer (%zu bytes, version %s)\n",
            __func__, n_kv, n_tensors, size, llama_file_version_name(fver));
    ...
    // Buffer loading never uses mmap or direct I/O
    this->use_mmap = false;
...
void llama_model_loader::load_data_for(struct ggml_tensor * cur) const {
    ...
    } else if (buf_data != nullptr) {
        // Buffer-backed loading: copy directly from buffer
        GGML_ASSERT(cur->data != nullptr);
        GGML_ASSERT(w.offs + ggml_nbytes(cur) <= buf_size);
        memcpy(cur->data, buf_data + w.offs, ggml_nbytes(cur));
    } else {
        GGML_ASSERT(cur->data != nullptr);
...
bool llama_model_loader::load_all_data(
    ...
    for (struct ggml_tensor * cur = ggml_get_first_tensor(ctx); cur != NULL; cur = ggml_get_next_tensor(ctx, cur)) {
        ...
        if (use_mmap) {
            ...
            // Buffer-backed loading: copy directly from buffer
            GGML_ASSERT(weight->offs + n_size <= buf_size);
            const uint8_t * src = buf_data + weight->offs;

            if (ggml_backend_buffer_is_host(cur->buffer)) {
                memcpy(cur->data, src, n_size);
                if (check_tensors) {
