...
static int llama_model_load_from_buffer_internal(const void * data, size_t size, llama_model & model, llama_model_params & params) {
    model.t_load_us = 0;
    time_meas tm(model.t_load_us);
    ...
    try {
        llama_model_loader ml(data, size, params.check_tensors, params.no_alloc, params.kv_overrides, params.tensor_buft_overrides);
...
struct llama_model * llama_model_load_from_buffer(
    ...
    if (!data || size == 0) {
        LLAMA_LOG_ERROR("%s: invalid buffer (data=%p, size=%zu)\n", __func__, data, size);
        return nullptr;
    }
    ...
    // device setup (same as file-based loading)
    if (params.devices) {
        for (ggml_backend_dev_t * dev = params.devices; *dev; ++dev) {
            model->devices.push_back(*dev);
        }
    ...
    const int status = llama_model_load_from_buffer_internal(data, size, *model, params);
    GGML_ASSERT(status <= 0);
    if (status < 0) {
        if (status == -1) {
            LLAMA_LOG_ERROR("%s: failed to load model from buffer\n", __func__);
        } else if (status == -2) {
            LLAMA_LOG_INFO("%s: cancelled model load\n", __func__);
        }
        llama_model_free(model);
