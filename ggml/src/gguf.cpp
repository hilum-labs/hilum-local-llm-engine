...
struct gguf_reader {
    ...
    // Get current position (for alignment calculations)
    size_t tell() const {
        return static_cast<size_t>(gguf_ftell(file));
    }

    // Seek to absolute position
    bool seek(size_t pos) const {
        if (gguf_fseek(file, pos, SEEK_SET) != 0) {
            return false;
        }
        nbytes_remain = file_remain(file);
        return true;
...
struct gguf_buf_reader {
    gguf_buf_reader(const uint8_t * data, size_t size) : data(data), size(size), offset(0) {}
    ...
    bool read(std::vector<T> & dst, const size_t n) const {
        ...
        if constexpr (std::is_same<T, std::string>::value) {
            ...
            if (size - offset < n * sizeof(uint64_t)) {
                return false;
            }
        } else {
            if (n > SIZE_MAX / sizeof(T)) {
    ...
    // Get current position (analogous to ftell)
...
bool gguf_read_emplace_helper(const ReaderT & gr, std::vector<struct gguf_kv> & kv, const std::string & key, const bool is_array, const size_t n) {
    if (is_array) {
        std::vector<T> value;
        try {
            if (!gr.read(value, n)) {
...
static struct gguf_context * gguf_init_impl(const ReaderT & gr, struct gguf_init_params params) {
...
struct gguf_context * gguf_init_from_buffer_impl(const void * data, size_t size, struct gguf_init_params params) {
    const struct gguf_buf_reader gr(static_cast<const uint8_t *>(data), size);
    return gguf_init_impl(gr, params);
}
...
struct gguf_context * gguf_init_from_buffer(const void * data, size_t size, struct gguf_init_params params) {
    if (!data || size == 0) {
        GGML_LOG_ERROR("%s: invalid buffer (data=%p, size=%zu)\n", __func__, data, size);
        return nullptr;
    }
    return gguf_init_from_buffer_impl(data, size, params);
}
