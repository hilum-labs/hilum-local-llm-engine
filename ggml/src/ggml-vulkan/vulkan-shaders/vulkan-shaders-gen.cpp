...
void matmul_shaders(bool fp16, MatMulIdType matmul_id_type, bool coopmat, bool coopmat2, bool f16acc, bool adreno = false) {
    ...
    if (matmul_id_type == MatMulIdType::DEFAULT) {
        ...
        shader_name = device_prefix + "matmul_id";
    } else if (matmul_id_type == MatMulIdType::SUBGROUP) {
        base_dict["MUL_MAT_ID"] = "1";
        base_dict["MUL_MAT_ID_USE_SUBGROUPS"] = "1";
        shader_name = device_prefix + "matmul_id_subgroup";
    } else {
        shader_name = device_prefix + "matmul";
    }
...
void process_shaders() {
    ...
    // Adreno-specific shader variants
#ifdef GGML_VULKAN_BUILD_ADRENO_SHADERS
    {
        ...
        for (const auto& tname : adreno_shader_types) {
            ...
            // get_rows shaders (for quant types only)
            if (tname != "f32" && tname != "f16") {
                std::string get_rows_shader = "get_rows_quant.comp";
                string_to_spv(device_pfix + "get_rows_" + tname, get_rows_shader, merge_maps(adreno_base_dict, {{"TEMP_TYPE", "FLOAT_TYPE"}, {data_a_key, "1"}, {"B_TYPE", "int"}, {"D_TYPE", "float16_t"}}));
            }
        }

        // copy_from_quant shaders
        for (std::string tname : {"q4_0", "q4_1", "q8_0"}) {
            string_to_spv(device_pfix + "cpy_" + tname + "_f32", "copy_from_quant.comp", merge_maps(adreno_base_dict, {{"DATA_A_" + to_uppercase(tname), "1"}, {"D_TYPE", "float"}, {"FLOAT_TYPE", "float"}}));
        }

        // quantize shaders
        string_to_spv(device_pfix + "quantize_q8_1", "quantize_q8_1.comp", {{"ADRENO", "1"}});
        string_to_spv(device_pfix + "quantize_q8_1_x4", "quantize_q8_1.comp", {{"ADRENO", "1"}, {"QBLOCK_X4", "1"}});
...
void write_output_files() {
    ...
    // Adreno variant lookup infrastructure
    ...
    for (const auto &pair : shader_fnames) {
        ...
        if (string_starts_with(nm, "adreno_")) {
            adreno_bases.push_back(nm.substr(std::string("adreno_").size()));
        }
    }
