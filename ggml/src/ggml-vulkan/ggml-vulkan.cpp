...
struct vk_command_pool {
    ...
    size_t buffers_in_use() const {
        return std::count_if(cmd_buffers.begin(), cmd_buffers.end(), [](const auto & cb) {
            return cb.in_use;
        });
    }
};
...
enum vk_device_architecture {
    ...
    NVIDIA_PRE_TURING,
    NVIDIA_TURING,
    ADRENO_6XX,  // Snapdragon 8xx/7xx (2019-2021)
    ADRENO_7XX,  // Snapdragon 8 Gen 1/2 (2022-2023)
    ADRENO_8XX,  // Snapdragon 8 Gen 3+ (2024+)
};
...
static vk_device_architecture get_device_architecture(const vk::PhysicalDevice& device) {
    ...
    if (props.vendorID == VK_VENDOR_ID_AMD) {
        ...
        // Qualcomm Adreno GPUs - detect series from device name
        // Device names follow pattern "Adreno (TM) 730", "Adreno (TM) 640", etc.
        const std::string device_name(props.deviceName);
        // Find the series digit: look for a 3-digit number starting with 6, 7, or 8
        for (size_t i = 0; i + 2 < device_name.size(); ++i) {
            if (std::isdigit(device_name[i]) && std::isdigit(device_name[i+1]) && std::isdigit(device_name[i+2])) {
                char series = device_name[i];
                if (series == '8') {
                    return vk_device_architecture::ADRENO_8XX;
                } else if (series == '7') {
                    return vk_device_architecture::ADRENO_7XX;
                } else if (series == '6') {
                    return vk_device_architecture::ADRENO_6XX;
                }
                break;
            }
        }
...
struct vk_device_struct {
    ...
    uint64_t ssbo_cumulative_limit;  // 0 = no limit; Adreno: descriptorBufferAddressSpaceSize
...
struct vk_buffer_struct {
    ...
    ~vk_buffer_struct() {
        ...
        if (vma_allocation != VK_NULL_HANDLE) {
            vmaDestroyBuffer(device->vma_allocator, static_cast<VkBuffer>(buffer), vma_allocation);
            return;
        }
#endif
...
struct ggml_backend_vk_context {
    ...
    size_t prealloc_size_x, prealloc_size_y, prealloc_size_split_k, prealloc_size_add_rms_partials, prealloc_size_add_rms_partials_offset, prealloc_tile_buf_size;
    vk_buffer prealloc_x, prealloc_y, prealloc_split_k, prealloc_add_rms_partials, prealloc_tile_buf, sync_staging;
...
#ifdef GGML_VULKAN_VMA
    if (device->vma_allocator != VK_NULL_HANDLE && !import_ptr) {
        VkBufferCreateInfo vma_bci = {};
        vma_bci.sType = VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO;
        vma_bci.size = size;
        vma_bci.usage = VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_SRC_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT;
        if (device->buffer_device_address) {
            vma_bci.usage |= VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT;
        }

        VmaAllocationCreateInfo vma_aci = {};
        // Determine memory usage from requested flags
        const auto & first_flags = *req_flags_list.begin();
        if (first_flags & vk::MemoryPropertyFlagBits::eDeviceLocal) {
            vma_aci.usage = VMA_MEMORY_USAGE_GPU_ONLY;
        } else if (first_flags & vk::MemoryPropertyFlagBits::eHostVisible) {
            vma_aci.usage = VMA_MEMORY_USAGE_CPU_TO_GPU;
            vma_aci.flags = VMA_ALLOCATION_CREATE_MAPPED_BIT;
        } else {
            vma_aci.usage = VMA_MEMORY_USAGE_AUTO;
        }

        VkBuffer vk_buffer;
        VmaAllocation allocation;
        VmaAllocationInfo alloc_info;
        VkResult result = vmaCreateBuffer(device->vma_allocator, &vma_bci, &vma_aci, &vk_buffer, &allocation, &alloc_info);
        if (result != VK_SUCCESS) {
            throw vk::OutOfDeviceMemoryError("VMA allocation failed");
        }

        buf->buffer = vk::Buffer(vk_buffer);
        buf->vma_allocation = allocation;
        buf->memory_property_flags = first_flags;
...
static vk_fa_tuning_params get_fa_tuning_params_scalar(const vk_device& device, uint32_t hsk, uint32_t hsv, uint32_t n_rows, uint32_t n_kv, ggml_type kv_type, bool f32acc) {
    ...
    if (device->vendor_id == VK_VENDOR_ID_QUALCOMM) {
        // Adreno GPUs: use conservative subgroup settings, disable subgroup ops for stability
        result.subgroup_size = 64;
...
static void ggml_vk_load_shaders(vk_device& device) {
    ...
    if (device->coopmat2) {
        ...
        if ((device->architecture == AMD_GCN) && (device->driver_id != vk::DriverId::eAmdProprietary)) {
            ...
            // Adreno GPUs: smaller tile sizes to avoid register pressure on mobile
            l_warptile     = { 64,  64,  64, 16, subgroup_size_8, 32, 2, tm_s, tn_s, tk_s, subgroup_size_8 };
            m_warptile     = { 64,  32,  32, 16, subgroup_size_8, 32, 2, tm_s, tn_s, tk_s, subgroup_size_8 };
            l_warptile_mmq = { 64,  64,  64, 32, subgroup_size_8, 32, 2, tm_s, tn_s, tk_s, subgroup_size_8 };
            m_warptile_mmq = { 64,  32,  32, 32, subgroup_size_8, 32, 2, tm_s, tn_s, tk_s, subgroup_size_8 };
        }
...
#undef CREATE_MM
    ...
    const bool is_adreno = device->vendor_id == VK_VENDOR_ID_QUALCOMM;
    ...
    // Adreno: disable fused rms_norm pipelines (produce incorrect results on some drivers)
    if (device->vendor_id == VK_VENDOR_ID_QUALCOMM) {
        device->pipeline_rms_norm_mul_f32 = {};
        device->pipeline_rms_norm_mul_partials_f32 = {};
        device->pipeline_rms_norm_mul_rope_f32_f32 = {};
        device->pipeline_rms_norm_mul_rope_f32_f16 = {};
    }
...
static vk_device ggml_vk_get_device(size_t idx) {
    ...
    if (vk_instance.devices[idx] == nullptr) {
        ...
        // Adreno GPU capability workarounds
        if (device->architecture == ADRENO_6XX) {
            // Adreno 6xx has known driver bugs with subgroup shuffle, clustered ops, and cooperative matrix
            device->subgroup_shuffle = false;
            device->subgroup_clustered = false;
            device->subgroup_arithmetic = false;  // buggy on 6XX
            device->subgroup_ballot = false;      // unreliable on 6XX
            device->coopmat_support = false;
        } else if (device->architecture == ADRENO_7XX) {
            // Adreno 7xx cooperative matrix is not reliably supported
            device->coopmat_support = false;
            device->subgroup_ballot = false;      // unreliable on some 7XX
        }
        // All Adreno: force conservative subgroup require
        if (device->vendor_id == VK_VENDOR_ID_QUALCOMM) {
            device->subgroup_require_full_support = false;
        }

        // Adreno: cumulative SSBO size limit from descriptor buffer address space
        device->ssbo_cumulative_limit = 0;
        if (descriptor_buffer_support && getenv("GGML_VK_NO_SSBO_TILING") == nullptr &&
            device->vendor_id == VK_VENDOR_ID_QUALCOMM) {
            device->ssbo_cumulative_limit = descriptor_buffer_props.descriptorBufferAddressSpaceSize;
        }
...
#ifdef GGML_VULKAN_VMA
        {
            ...
            vma_ci.physicalDevice = device->physical_device;
            vma_ci.device = device->device;
            vma_ci.instance = vk_instance.instance;
            if (device->vendor_id == VK_VENDOR_ID_QUALCOMM) {
                vma_ci.flags |= VMA_ALLOCATOR_CREATE_KHR_DEDICATED_ALLOCATION_BIT;
            }
            vmaCreateAllocator(&vma_ci, &device->vma_allocator);
        }
#endif
        ...
        for (uint32_t i = 0; i < GGML_TYPE_COUNT; ++i) {
            ...
            case VK_VENDOR_ID_QUALCOMM:
                device->mul_mat_l[i] = false;      // large matmul crashes Adreno drivers
                device->mul_mat_id_l[i] = false;
                device->mul_mat_m[i] = true;
                device->mul_mat_s[i] = true;
                device->mul_mat_id_m[i] = true;
...
static uint32_t ggml_vk_guess_split_k(ggml_backend_vk_context * ctx, uint32_t m, uint32_t n, uint32_t k, bool disable_split_k, const vk_pipeline& pipeline) {
    ...
    // Adreno: disable split_k since tiling already handles large matrices
    // and split_k would increase memory requirements
...
// Adreno SSBO tiling helpers

static void ggml_vk_post_compute_to_transfer_barrier(vk_context& subctx, vk_buffer& buf, size_t offset, size_t size) {
...
static void ggml_vk_transfer_to_compute_barrier(vk_context& subctx, vk_buffer& buf, size_t offset, size_t size) {
    VkBufferMemoryBarrier barrier{};
    barrier.sType = VK_STRUCTURE_TYPE_BUFFER_MEMORY_BARRIER;
...
static void ggml_vk_copy_2d(vk_context& subctx, vk_buffer& dst, size_t dst_offset,
                             vk_buffer& src, size_t src_offset,
                             size_t width, size_t height,
                             size_t spitch, size_t dpitch) {
    VK_LOG_DEBUG("ggml_vk_copy_2d(dst_offset=" << dst_offset
    ...
    // Adreno drivers can choke on very large vkCmdCopyBuffer calls; batch in groups of 256
    const int group_size = 256;
    const int groups = CEIL_DIV((int)height, group_size);
    ...
    for (int i = 0; i < groups; i++) {
        ...
        for (size_t row = i * group_size; row < (size_t)i * group_size + group_height; ++row) {
            VkBufferCopy bc{ src_offset + row * spitch, dst_offset + row * dpitch, width };
        ...
        ggml_vk_ctx_end(subctx);
        ggml_vk_submit(subctx, src->device->fence);
        VK_CHECK(src->device->device.waitForFences({ src->device->fence }, true, UINT64_MAX), "vk_copy_2d wait");
        src->device->device.resetFences({ src->device->fence });
...
static uint64_t total_ssbo_bytes(uint64_t m, uint64_t n, uint64_t k,
                                 enum ggml_type m_type, enum ggml_type n_type, enum ggml_type d_type) {
...
// Compute largest tile dimensions (mt, nt) such that tiled A + B + D fits under the SSBO limit.
// Grows both M and N in steps of SSBO_TILE_STEP until the budget is exhausted.
static void compute_tile_dimensions(ggml_backend_vk_context * ctx,
                                    uint64_t m, uint64_t n, uint64_t k,
                                    uint64_t *out_mt, uint64_t *out_nt,
                                    uint64_t *out_m_tiles, uint64_t *out_n_tiles,
                                    uint64_t *out_num_dispatches,
                                    enum ggml_type m_type = GGML_TYPE_F32,
                                    enum ggml_type n_type = GGML_TYPE_F32,
                                    enum ggml_type d_type = GGML_TYPE_F32) {
    VK_LOG_DEBUG("compute_tile_dimensions(m=" << m << ", n=" << n << ", k=" << k
        << ", m_type=" << ggml_type_name(m_type)
        << ", n_type=" << ggml_type_name(n_type)
        << ", d_type=" << ggml_type_name(d_type) << ")");

    const uint64_t step = SSBO_TILE_STEP;
    uint64_t mt = std::min(step, m);
    uint64_t nt = std::min(step, n);

    GGML_ASSERT(total_ssbo_bytes(mt, nt, k, m_type, n_type, d_type) < ctx->device->ssbo_cumulative_limit);
    ...
    while (nt < n || mt < m) {
        ...
        if (nt < n) {
            ...
            if (total_ssbo_bytes(mt, next_nt, k, m_type, n_type, d_type) < ctx->device->ssbo_cumulative_limit) {
                nt = next_nt;
            } else {
                nt_stopped = true;
            }
        ...
        if (mt < m) {
            ...
            if (total_ssbo_bytes(next_mt, nt, k, m_type, n_type, d_type) < ctx->device->ssbo_cumulative_limit) {
                mt = next_mt;
            } else {
                mt_stopped = true;
            }
    ...
    *out_mt = mt;
    *out_nt = nt;
    *out_m_tiles = CEIL_DIV(m, mt);
    *out_n_tiles = CEIL_DIV(n, nt);
    *out_num_dispatches = (*out_m_tiles) * (*out_n_tiles);
}

// Tiled matmul dispatch: copies sub-tiles of A and B into the preallocated tile buffer,
// dispatches matmul per tile, and copies results back to the destination buffer.
static void ggml_vk_matmul_tiled_dispatch(ggml_backend_vk_context *ctx, vk_context& subctx, vk_pipeline& pipeline,
        const uint64_t ne00, const uint64_t ne01, const uint64_t ne10, const uint64_t ne11, const uint64_t ne20,
        ggml_type a_type, ggml_type b_type, ggml_type d_type, const uint64_t mt, const uint64_t nt,
        vk_buffer& d_X, size_t x_buf_offset,
        vk_buffer& d_Y, size_t y_buf_offset,
        vk_buffer& d_D, size_t d_buf_offset) {
    VK_LOG_DEBUG("ggml_vk_matmul_tiled_dispatch(ne00=" << ne00 << ", ne01=" << ne01
        << ", ne10=" << ne10 << ", ne11=" << ne11
        << ", a_type=" << ggml_type_name(a_type) << ", b_type=" << ggml_type_name(b_type)
        << ", d_type=" << ggml_type_name(d_type) << ", mt=" << mt << ", nt=" << nt << ")");

    const uint32_t a_bytes_per_block = ggml_type_size(a_type);
    const uint32_t a_elems_per_block = ggml_blck_size(a_type);
    const uint32_t b_bytes_per_block = ggml_type_size(b_type);
    const uint32_t b_elems_per_block = ggml_blck_size(b_type);
    const uint32_t d_bytes_per_block = ggml_type_size(d_type);
    const uint32_t d_elems_per_block = ggml_blck_size(d_type);

    for (uint32_t n0 = 0; n0 < ne11; n0 += nt) {
        const uint32_t cur_nt = (uint32_t)std::min(nt, ne11 - n0);
        const uint64_t b_off_bytes = y_buf_offset + CEIL_DIV(((uint64_t)n0 * ne10), b_elems_per_block) * b_bytes_per_block;
        const uint64_t d_off_bytes_n = d_buf_offset + CEIL_DIV(((uint64_t)n0 * ne20), d_elems_per_block) * d_bytes_per_block;

        const uint64_t b_k_bytes = CEIL_DIV(ne10, b_elems_per_block) * b_bytes_per_block;
        const uint64_t b_stride = b_k_bytes;
        const uint64_t b_tile_stride = b_stride;
        const uint32_t b_tile_stride_elems = (b_tile_stride / b_bytes_per_block) * b_elems_per_block;
        const uint64_t b_tile_bytes = b_tile_stride * cur_nt;
        const uint64_t b_off = 0;

        // copy B tile into tile buffer
        ggml_vk_copy_2d(subctx, ctx->prealloc_tile_buf, b_off, d_Y, b_off_bytes, b_k_bytes, cur_nt, b_stride, b_tile_stride);

        for (uint32_t m0 = 0; m0 < ne01; m0 += mt) {
            const uint32_t cur_mt = (uint32_t)std::min(mt, ne01 - m0);
            const uint64_t a_off_bytes = x_buf_offset + CEIL_DIV(((uint64_t)m0 * ne00), a_elems_per_block) * a_bytes_per_block;
            const uint64_t d_off_bytes = d_off_bytes_n + CEIL_DIV((uint64_t)m0, d_elems_per_block) * d_bytes_per_block;

            const uint64_t a_k_bytes = CEIL_DIV(ne00, a_elems_per_block) * a_bytes_per_block;
            const uint64_t a_stride = a_k_bytes;
            const uint64_t a_tile_stride = a_stride;
            const uint64_t d_orig_stride = CEIL_DIV(ne20, d_elems_per_block) * d_bytes_per_block;
            const uint64_t d_tile_stride = CEIL_DIV(cur_mt, d_elems_per_block) * d_bytes_per_block;

            const uint32_t a_tile_stride_elems = (a_tile_stride / a_bytes_per_block) * a_elems_per_block;
            const uint32_t d_tile_stride_elems = (d_tile_stride / d_bytes_per_block) * d_elems_per_block;
            const uint64_t dst_copy_row_size = d_tile_stride;

            const uint64_t a_tile_bytes = a_tile_stride * cur_mt;
            const uint64_t d_tile_bytes = d_tile_stride * cur_nt;

            const uint64_t a_off = b_off + b_tile_bytes;
            const uint64_t d_off = a_off + a_tile_bytes;

            GGML_ASSERT(a_tile_bytes + b_tile_bytes + d_tile_bytes < ctx->device->ssbo_cumulative_limit);

            // copy A tile into tile buffer
            ggml_vk_copy_2d(subctx, ctx->prealloc_tile_buf, a_off, d_X, a_off_bytes, a_k_bytes, cur_mt, a_stride, a_tile_stride);
            ggml_vk_transfer_to_compute_barrier(subctx, ctx->prealloc_tile_buf, 0, a_tile_bytes + b_tile_bytes);

            // dispatch matmul for this tile
            ggml_vk_matmul(
                ctx, subctx, pipeline,
                { ctx->prealloc_tile_buf, a_off, a_tile_bytes }, { ctx->prealloc_tile_buf, b_off, b_tile_bytes },
                { ctx->prealloc_tile_buf, d_off, d_tile_bytes }, { nullptr, 0, 0 },
                cur_mt, cur_nt, (uint32_t)ne00,
                a_tile_stride_elems, b_tile_stride_elems, d_tile_stride_elems,
                a_tile_stride_elems * cur_mt, b_tile_stride_elems * cur_nt, d_tile_stride_elems * cur_nt,
                1u, 1u, 1u, 1u, 1u, 1u, cur_nt
            );

            // copy results back
            ggml_vk_post_compute_to_transfer_barrier(subctx, ctx->prealloc_tile_buf, d_off, d_tile_bytes);
            ggml_vk_copy_2d(subctx, d_D, d_off_bytes, ctx->prealloc_tile_buf, d_off, dst_copy_row_size, cur_nt, d_tile_stride, d_orig_stride);
...
static void ggml_vk_mul_mat_q_f16(ggml_backend_vk_context * ctx, vk_context& subctx, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst, bool disable_split_k) {
    ...
    // Adreno SSBO tiling: check if cumulative buffer sizes exceed driver limit
    const uint64_t ssbo_limit = ctx->device->ssbo_cumulative_limit;
    const bool needs_ssbo_tiling = ssbo_limit > 0 &&
        (ne02 == 1 && ne03 == 1 && ne12 == 1 && ne13 == 1) &&
        (x_sz * ne02 * ne03 + y_sz * ne12 * ne13 + d_sz * ne12 * ne13 >= ssbo_limit);

    uint64_t tile_mt = 0, tile_nt = 0, m_tile_count = 0, n_tile_count = 0, tile_dispatches = 1;
    if (needs_ssbo_tiling) {
        ggml_type eff_a_type = qx_needs_dequant ? f16_type : src0->type;
        ggml_type eff_b_type = quantize_y ? GGML_TYPE_Q8_1 : (y_f32_kernel ? GGML_TYPE_F32 : f16_type);
        compute_tile_dimensions(ctx, ne01, ne11, ne00, &tile_mt, &tile_nt, &m_tile_count, &n_tile_count, &tile_dispatches, eff_a_type, eff_b_type);
        VK_LOG_DEBUG("[ggml_vk_mul_mat_q_f16] [ssbo_tiling] tile_mt="
            << tile_mt << ", tile_nt=" << tile_nt << ", m_tiles=" << m_tile_count
            << ", n_tiles=" << n_tile_count << ", dispatches=" << tile_dispatches);
    }

    {
        ...
        if (needs_ssbo_tiling) {
            ctx->prealloc_tile_buf_size = ssbo_limit;
            GGML_ASSERT(split_k <= 1);
            ggml_vk_preallocate_buffers(ctx, subctx);
        }

        // Request descriptor sets
        ggml_pipeline_request_descriptor_sets(ctx, pipeline, tile_dispatches);
        if (qx_needs_dequant) {
    ...
    if (needs_ssbo_tiling) {
        ...
        ggml_type eff_b_type = quantize_y ? GGML_TYPE_Q8_1 : (y_f32_kernel ? GGML_TYPE_F32 : f16_type);
        GGML_ASSERT(split_k <= 1);
        ggml_vk_matmul_tiled_dispatch(ctx, subctx, pipeline,
            ne00, ne01, ne10, ne11, ne10,
            eff_a_type, eff_b_type, GGML_TYPE_F32, tile_mt, tile_nt,
...
static void ggml_vk_preallocate_buffers(ggml_backend_vk_context * ctx, vk_context subctx) {
    ...
    if (ctx->prealloc_tile_buf == nullptr || (ctx->prealloc_tile_buf_size > 0 && ctx->prealloc_tile_buf->size < ctx->prealloc_tile_buf_size)) {
        VK_LOG_MEMORY("ggml_vk_preallocate_buffers(tile_buf_size: " << ctx->prealloc_tile_buf_size << ")");
        // Resize buffer
        if (ctx->prealloc_tile_buf != nullptr) {
            ggml_vk_destroy_buffer(ctx->prealloc_tile_buf);
        }
        ctx->prealloc_tile_buf = ggml_vk_create_buffer_device(ctx->device, ctx->prealloc_tile_buf_size);
    }
}
...
static void ggml_backend_vk_set_tensor_async(ggml_backend_t backend, ggml_tensor * tensor, const void * data, size_t offset, size_t size) {
    ...
    if (!ret) {
        ...
        compute_ctx->s->buffer->buf.copyBuffer(ctx->sync_staging->buffer, buf->buffer, { buffer_cpy });
...
static void ggml_backend_vk_event_record(ggml_backend_t backend, ggml_backend_event_t event) {
    ...
    auto * cmd_buf = compute_ctx->s ? compute_ctx->s->buffer : nullptr;
